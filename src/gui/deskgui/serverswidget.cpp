    ...
    , m_serverInstanceInfoConnection()
{
    ...
    // Actions for creating, editing and deleting Servers
    toolBar->addAction(
        tr("New Server"),
        [this](void) { createNewServer(); });

    // Editing action is associated with both the editing button and the
    // table double-click
    auto editServerFn = [this](void) { editExistingServer(); };
...
void ServersWidget::createNewServer(void)
{
    TD_ACTION_TRY
    {
...
}   // end createNewServer method

void ServersWidget::editExistingServer(void)
{
    TD_ACTION_TRY
    {
...
}   // end editExistingServer method

void ServersWidget::deleteServer(void)
{
